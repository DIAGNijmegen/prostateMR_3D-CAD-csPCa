...
prsr.add_argument('--NAME',                       type=str,   default='diagnosis/',                                                        help='Path to Load/Store Model Weights and Performance Metrics')
prsr.add_argument('--NUM_EPOCHS',                 type=int,   default=250,                                                                 help="Number of Training Epochs")
...
prsr.add_argument('--WEIGHTS_MIN_EPOCH',          type=int,   default=5,                                                                   help="Minimum Epoch to Start Exporting Weights")
...
prsr.add_argument('--VALIDATE_MIN_EPOCH',         type=int,   default=5,                                                                   help="Minimum Epoch to Start Validation")
prsr.add_argument('--SHOW_SUMMARY',               type=int,   default=0,                                                                   help="Display Overview")
prsr.add_argument('--RESUME_TRAIN',               type=int,   default=0,                                                                   help="Enable Resume Training (Experimental)")
...
prsr.add_argument('--BATCH_SIZE',          type=int,   default=2,                                                                                help="Batch Size")
...
prsr.add_argument('--CLR_PARAMS',          type=float, default=[5e-5, 1.00, 1.25],                                                    nargs='+', help="'CyclicLR': Max LR, Decay Factor, Step Factor")
...
prsr.add_argument('--DSC_BD_LOSS_WEIGHTS', type=float, default=[0.50, 0.50],                                                          nargs='+', help="Soft Dice + Boundary Loss (weights)")
prsr.add_argument('--ELBO_LOSS_PARAMS',    type=float, default=[10],                                                                  nargs='+', help="Evidence Lower Bound Loss for Prob Dist. (weight)")
prsr.add_argument('--AUGM_PARAMS',         type=float, default=[1.00, 0.25, 0.15, 10.0, True, 1.20, 0.10, 0.025, True, [0.50, 1.50]], nargs='+', help="Train-Time Augmentations (M_PROB,TX_PROB,TRANS,ROT,HFLIP,SCALE,\
...
for f in args.FOLDS:
    ...
    train_data_gen = custom_data_generator(data_xlsx=TRAIN_XLSX, train_obj=args.TRAIN_OBJ, probabilistic=bool(args.UNET_PROBABILISTIC), mode='train')
    train_metrics  = custom_data_generator(data_xlsx=TRAIN_XLSX, train_obj=args.TRAIN_OBJ, probabilistic=bool(args.UNET_PROBABILISTIC), mode='valid')
    valid_data_gen = custom_data_generator(data_xlsx=VALID_XLSX, train_obj=args.TRAIN_OBJ, probabilistic=bool(args.UNET_PROBABILISTIC), mode='valid')
    print("Complete.")
    #----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
